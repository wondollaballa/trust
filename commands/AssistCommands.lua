...
function AssistTrustCommands:handle_kite_assist()
    local assistant = nil
    local party = self.trust and self.trust.get_party and self.trust:get_party()
    local debug_msgs = {}

    if party then
        -- Try .get_roles() method first
        if party.get_roles then
            local roles = party:get_roles()
            table.insert(debug_msgs, "party:get_roles() returned type: "..type(roles))
            if type(roles) == "table" then
                for k, role in pairs(roles) do
                    if type(role) == "table" and role.get_type and role:get_type() == "assistant" then
                        assistant = role
                        break
                    end
                end
        ...
        -- Fallback: try .roles table directly
        if not assistant and type(party.roles) == "table" then
            table.insert(debug_msgs, "party.roles is a table with keys: "..table.concat((function(t) local r = {}; for k,_ in pairs(t) do table.insert(r, tostring(k)); end; return r end)(party.roles), ", "))
            for k, role in pairs(party.roles) do
                if type(role) == "table" and role.get_type and role:get_type() == "assistant" then
                    assistant = role
                    break
                end
    ...
    if not assistant or not assistant.handle_kite_assist_command then
        ...
        if party then
            for _, msg in ipairs(debug_msgs) do
                self.trust:get_party():add_to_chat(self.trust:get_party():get_player(), "[KiteAssist Debug] "..msg)
            end
            -- List all available role types
            local role_types = {}
            local roles = (party.get_roles and party:get_roles()) or party.roles or {}
            for k, role in pairs(roles) do
                if type(role) == "table" and role.get_type then
                    table.insert(role_types, tostring(k).."="..tostring(role:get_type()))
                end
            end
            self.trust:get_party():add_to_chat(self.trust:get_party():get_player(),
                "KiteAssist error: Available roles: " .. table.concat(role_types, ", "))
        end
